"""
Telegram Animatsion Emoji Bot — aiogram 3.x
=============================================
O'rnatish:
    pip install "aiogram>=3.0" fonttools

Ishlatish:
    python emoji_bot_final.py
"""
...
TEMPLATES = {
    "1": ("🌟 Dizayn 1", TEMPLATE_1_B64),
    "2": ("✨ Dizayn 2",  TEMPLATE_2_B64),
}
...
user_state: dict[int, str] = {}  # {user_id: text}


def design_keyboard() -> InlineKeyboardMarkup:
    buttons = [
        [InlineKeyboardButton(text=label, callback_data=f"design:{key}")]
...
@dp.callback_query(F.data.startswith("design:"))
async def handle_design(callback: CallbackQuery):
    uid        = callback.from_user.id
    design_key = callback.data.split(":")[1]
    user_text  = user_state.get(uid)
    ...
    try:
        ...
        tgs_bytes = generate_tgs(user_text.upper(), template_b64)

        buf      = BytesIO(tgs_bytes)
        buf.name = f"{user_text.upper()}.tgs"

        await callback.message.answer_document(
            buf,
            caption=(
                f"✨ <b>{user_text.upper()}</b> emojingiz tayyor!\n\n"
...
async def main():
    logging.info("Bot ishga tushdi (aiogram 3.x)...")
